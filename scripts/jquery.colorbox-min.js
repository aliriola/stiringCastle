...
(function(c) {
    ...
    function Y() {
        for (var b in a)
            if (c.isFunction(a[b]) && b.substring(0, 2) !== "on")
                a[b] = a[b].call(i);
        a.rel = a.rel || i.rel;
    ...
    var q = "colorbox", F = "hover", n = true, e, x=!c.support.opacity, O = x&&!window.XMLHttpRequest, aa = "cbox_open", H = "cbox_load", S = "cbox_complete", T = "resize.cbox_resize", s, k, u, p, U, V, W, X, g, m, l, I, J, K, Q, P, t, E, D, R, y, z, v, w, i, N, j, a, B, C, $ = {
        transition: "elastic",
        speed: 350,
        width: false,
        height: false,
    ...
    e.init = function() {
        ...
        c(".cboxElement").live("click", function(d) {
            if (d.button !== 0 && typeof d.button !== "undefined")
                return n;
            else {
                Z(this);
                return false
        ...
        function f(G) {
            e.position(G, function() {
                if (B) {
                    ...
                    if (a.iframe)
                        l.append("<iframe id='cboxIframe'" + (a.scrolling ? " " : "scrolling='no'") + " name='iframe_" + (new Date).getTime() + "' frameborder=0 src='" + a.href + "' " + (x ? "allowtransparency='true'" : "") + " />");
                    l.show();
                    Q.show().html(a.title);
                    if (g.length > 1) {
    ...
    e.load = function() {
        ...
        if (a.inline) {
            ...
            c("<div />").appendTo(I).load(b, function(o, A) {
